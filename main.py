...
class SearchHandler(webapp.RequestHandler):
  def get(self):
    ...
    if genre == u"all":
      logging.info("all")
      queryStores = u"SELECT * FROM Stores WHERE station = '%(reqstation)s'" % { 'reqstation': station }
    else:
      queryStores = u"SELECT * FROM Stores WHERE station = '%(reqstation)s' AND genre = '%(reqgenre)s'" % { 'reqstation': station, 'reqgenre': genre }
    ...
    queryStations = u"SELECT * FROM Stations WHERE name = '%(reqstation)s'" % { 'reqstation': station }
    ...
    resStation = {'name': station, 'lat': resultStations[0].lat.encode('utf-8'), 'lng': resultStations[0].lng.encode('utf-8')}
...
#    logging.info(stores)
    ...
    for store in resultStores:
      resStores.append({"name": store.name, "genre": store.genre.encode('utf-8'), "lat": store.lat.encode('utf-8'), "lng": store.lng.encode('utf-8')})
